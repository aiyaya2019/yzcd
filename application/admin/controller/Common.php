...
Class Common extends Base{
    ...
    public function selectAll($model,$data=[])
    {
        ...
        $info['list'] = $table->with($with)->where($where)->order($order)->field($field)->page($page,20)->select();
     ...
     * @return array
     */
    public function arrToOne($array) {
        $arr = array();
     ...
     * 生成券码
     */
    public function coupon_code($length = 18){
        $chars = "0123456789";  
        $str ="";
        for ( $i = 0; $i < $length; $i++ )  {  
            $str .= substr($chars, mt_rand(0, strlen($chars)-1), 1);  
        }
        $check = Coupon::where('coupon_code', $str)->find();
        if ($check) {
            $this->coupon_code();
        }else{
            return $str;
    ...
    public function getProvince(){
        $result = getAddressByTencent();
            // echo '<pre>';
            // preg_match("/[\d]{2}/i", '440000', $arr);
            // print_r($arr);exit;
        if ($result['status'] == '0') {
            $pre ="/[\d]{2}/i";
            foreach ($result['result']['0'] as $key => $value) {
                foreach ($result['result']['1'] as $k => $v) {
                    preg_match($pre, $value['id'], $arr1);
                    preg_match($pre, $v['id'], $arr2);
                    if ($arr1['0'] == $arr2['0']) {
                        $res[$key]['id'] = $value['id'];
                        $res[$key]['name'] = $value['fullname'];
                        $res[$key]['lat'] = $value['location']['lat'];
                        $res[$key]['lng'] = $value['location']['lng'];
                        // $res[$key]['city'][$k]['id'] = $v['id'];
                        // $res[$key]['city'][$k]['name'] = $v['fullname'];
                        // $res[$key]['city'][$k]['lat'] = $v['location']['lat'];
                        // $res[$key]['city'][$k]['lng'] = $v['location']['lng'];
                    }
                }
            }
            return $res;
    ...
    public function getCity(){
        if(request()->isAjax()){
            $post = input('post.');
            if (!$post['id']) { return_ajax(400, '缺少id'); }

            $result = getChildrenByTencent($post['id']);
            if ($result['status'] == '0') {
                foreach ($result['result']['0'] as $k => $v) {
                    $res[$k]['id']   = $v['id'];
                    $res[$k]['name'] = $v['fullname'];
                    // $res[$k]['lat']  = $v['location']['lat'];
                    // $res[$k]['lng']  = $v['location']['lng'];
                }
            }else{
