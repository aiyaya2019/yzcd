...
;layui.define(['jquery','form'],function(e){
    ...
    var table = [{name:'价格',key:'price'},{name:'市场价格',key:'old_price'},{name:'库存',key:'stock'}];
    ...
    "use strict";var $ = layui.jquery,form=layui.form,o ={
        ...
        st:function () {
            return '<table class="layui-table" lay-skin="" style="margin:0"><thead>'+this.stf()+this.sth()+'</thead><tbody>'+this.stb()+'</tbody></table>';
        },
        sth:function () {
            var h = '<tr>'; for(var i=0;i<this.c.tableTitle.length;i++){ h += '<th>' +this.c.tableTitle[i]+ '</th>';} 
        ...
        stb:function () {
            ...
            for(var i=0;i<t.c.data.length;i++){
                ...
                $(table).each(function(key,val){
                    h+='<td><input class="layui-input layui-table-input" data-index="'+i+'" data-type="'+val.key+'" lay-filter="input"  value="'+t.c.data[i][val.key]+'" ></td>'
                })
                h += '</tr>';
            }
            return h;
        ...
        stf:function () {
            ...
            var str = '';
                str += '<tr><th colspan="'+this.c.tableTitle.length+'" style="text-align: center">批量修改</th>';
                $(table).each(function(key,val){
                    str+='<th><input type="text" class="layui-input layui-table-input" lay-filter="input" data-type="sku-'+val.key+'" placeholder="'+val.name+'" /></th>'
                })
            return str;
        },
        gin:function (a,b) {
        ...
        cd:function () {
            ...
            for( var i=0;i<sarr.length;i++ ){
                ...
                $(table).each(function(k,val){
                    tarr[val.key] = rr[val.key];
                })
                narr.push(tarr);
            }
        ...
        ced:function(ids){
            var arr = [];arr['price']=0;arr['old_price']=0;arr['stock']=0;
            var nd = [];
            for(var i=0;i<ids.length;i++){
            ...
            for(var i=0;i<this.c.data.length;i++){
                ...
                if(is){
                    ...
                    $(table).each(function(k,val){
                        var key = val.key;
                        arr[key] = t.c.data[i][key];
                    })
                    //arr['price'] = parseFloat(this.c.data[i].price);
                    //arr['old_price'] = parseFloat(this.c.data[i].old_price);
                    //arr['stock'] = this.c.data[i].stock;
                    return arr;
                }
            }
            return arr;
        ...
        listenInput:function () {
            ...
            $('input[lay-filter]').on('change',function (e) {
                ...
                $(table).each(function(k,val){
                    if(p==val.key && t.c.data[i]) t.c.data[i][val.key] = v;
                    if(p=='sku-'+val.key){ for(var k=0;k<t.c.data.length;k++) t.c.data[k][val.key] = v;}
                })
                // if(p=='price' && t.c.data[i]) t.c.data[i].price = v;
                // if(p=='old_price' && t.c.data[i]) t.c.data[i].old_price = v;
                // if(p=='stock' && t.c.data[i]) t.c.data[i].stock = v;

                // if(p=='sku-price'){ for(var k=0;k<t.c.data.length;k++) t.c.data[k].price = v;}
                // if(p=='sku-old_price'){ for(var k=0;k<t.c.data.length;k++) t.c.data[k].old_price = v;}
                // if(p=='sku-stock'){ for(var k=0;k<t.c.data.length;k++) t.c.data[k].stock = v;}
                t.rl();
            })
        },
        listen:function () {
        ...
        l:function () {
            $('#'+this.c.sv).append('<div id="content">'+this.w(this.c.item,this.c.itemed,'')+'</div>');
            $('#'+this.c.sv).append('<div class="layui-form-item" pane id="skus"><label class="layui-form-label">商品SKU</label><div class="layui-input-block" id="'+this.c.tv+'">'+this.st()+'</div></div></div>');
            form.render('checkbox');this.listen();
        },
        init:function (c,tables='') {
            if(tables != ''){ table = tables; }
            this.f(c);
            this.l();
            return o;
        }
